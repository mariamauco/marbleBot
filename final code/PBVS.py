...
# Define a ROS2 node for publishing
class PBVSPublisher(Node):
    def __init__(self):
        super().__init__('pbvs_publisher')  # Pass 'pbvs_publisher' as the node name
        self.publisher_ = self.create_publisher(String, 'pbvs_data', 10)
        ...
        if len(corners) > 0 and ids is not None:
            ...
            for marker_idx in range(len(ids)):
                ...
                try:
                    ...
                    # Print the marker ID and the errors as a proof of concept
                   # print(f"Marker ID: {ids[marker_idx][0]}")
                    ...
                    info = f"Marker ID: {ids[marker_idx][0]}\n" \
                           f"  Translation Errors (in mm):\n" \
                           f"    x: {x_error:.2f}, y: {y_error:.2f}, z: {z_error:.2f}\n" \
                           f"  Rotation Errors (in degrees):\n" \
                           f"    Roll: {roll_error:.2f}, Pitch: {pitch_error:.2f}, Yaw: {yaw_error:.2f}\n" \
                           "------------------------------------------------------"
                    
            

                    # Publish info to ROS2
                    pbvs_publisher.publish_info(info)
            ...
            if args.show_gui:
                ...
                GUI.display_rotation_info(img=img_info, euler=estimated_euler_angles, euler_d=desired_euler_angles)
