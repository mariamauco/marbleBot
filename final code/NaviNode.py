...
DISTANCE_THRESHOLD = 1  # Distance threshold in meters for stopping near the marker
ORIENTATION_THRESHOLD = 10  # Rotation error threshold in degrees
...
class NavigationNode(Node):
    ...
    def marker_id_callback(self, msg):
        """Callback to update the TARGET_MARKER_ID."""
        global TARGET_MARKER_ID
        TARGET_MARKER_ID = msg.data  # Update the global variable
        self.get_logger().info(f"Updated TARGET_MARKER_ID: {TARGET_MARKER_ID}")
        print(f"[DEBUG]: TARGET_MARKER_ID updated to: {TARGET_MARKER_ID}")
    '''
    ...
    def trigger_solenoid(solenoid_pin = 7, duration=1):
 
        # Initialize the solenoid GPIO line
        solenoid_line = gpiod.Chip('gpiochip4').get_line(solenoid_pin)
        solenoid_line.request(consumer="solenoid_control", type=gpiod.LINE_REQ_DIR_OUT)
        
        try:
            # Set the GPIO pin high to activate the solenoid
            solenoid_line.set_value(1)
            print(f"Solenoid triggered on pin {solenoid_pin}.")
            
            # Wait for the specified duration
            time.sleep(duration)
            
            # Set the GPIO pin low to deactivate the solenoid
            solenoid_line.set_value(0)
            print(f"Solenoid deactivated on pin {solenoid_pin}.")
        finally:
